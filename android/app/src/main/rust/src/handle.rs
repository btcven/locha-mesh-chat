...
//! # Handle management
//!
//! To pass objects back and forth between Rust and Java we need to
//! put them in the Heap and use the pointer as a handle in the JNI.
...
use crate::util::unwrap_exc_or_default;

mod resource_manager;

/// Raw pointer passed from Java side.
pub type Handle = jlong;

/// "Converts" a type to a handle.
///
/// Returns a handle (raw pointer on the heap) to the given Java-owned object
/// allocated in the heap. This handle must be freed by `drop_handle` function call.
...
/// # Notes
///
/// Additional validity checks are performed if "runtime-typecheck" feature is enabled.
